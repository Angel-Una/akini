/*
 * akini-compat-bridge.js
 * 作用：把 Akini 的本地存储数据与 compat-main 的存储方案打通。
 * 说明：本文件作为兼容层引入，先完整迁移一次旧 Akini 数据到 compat 键名/格式，
 *       再拦截 Akini 对 localStorage 的写操作，使其同步写入 compat 格式。
 *       来信/回信逻辑也直接复用 compat 的 envelope（时空来信）核心。
 */
(function () {
  'use strict';
  ...
  window.__akinicompatBridgeReady = true;
  ...
  if (typeof localforage !== 'undefined') {
    try {
      ...
      console.warn('[akini-compat-bridge] localforage config failed', e);
    }
  }
  ...
  // 简单节流保存
  let _saveTimer = null;
  function throttledcompatSave(fn) {
  ...
  async function migrateAkiniTocompat() {
    ...
    try {
      ...
      // 2.2 单聊历史 → compat chatMessages（合并到 default session）
      const chatMessages = [];
      ...
      // 2.4 iCity 日记 → compat diary_*（仅保留数组型日记）
      ...
      console.log('[akini-compat-bridge] 迁移完成');
    } catch (e) {
      console.error('[akini-compat-bridge] 迁移失败', e);
    }
  }
  ...
  async function syncMailToEnvelope() {
    throttledcompatSave(async function () {
      try {
        let sent = [], received = [];
        const sentRaw = safeGetItem('akini_mail_sent');
  ...
  /* ========== 4. compat 来信/回信逻辑（精简版，适配 Akini 邮件 UI） ========== */
  ...
  function generateEnvelopeReplyText() {
    ...
    // 尝试从 compat 的 customReplies 读取；未引入时回退默认值
    try {
  ...
  // 暴露给 Akini 使用
  window.akinicompatBridge = {
    APP_PREFIX: APP_PREFIX,
    SESSION_ID: SESSION_ID,
    getStorageKey: getStorageKey,
    migrateAkiniTocompat: migrateAkiniTocompat,
  ...
  // 从 compat 的 localforage 恢复关键数据到 Akini localStorage（防止迁移后数据丢失）
  async function restoreFromcompatToAkini() {
    try {
      // 恢复 iCity 日记
      ...
      if (!Array.isArray(currentArr) || currentArr.length === 0) {
        const compatDiaries = await localforage.getItem(getStorageKey('diaryEntries'));
        if (compatDiaries && Array.isArray(compatDiaries) && compatDiaries.length > 0) {
          safeSetItem('akini_icity_diaries', JSON.stringify(compatDiaries));
          safeSetItem('akini_icity_diaries_backup', JSON.stringify(compatDiaries));
          console.log('[akini-compat-bridge] 从 localforage 恢复 iCity 日记:', compatDiaries.length);
        }
      }
      // 恢复朋友圈帖子
      ...
      if (!Array.isArray(postsArr) || postsArr.length === 0) {
        const compatPosts = await localforage.getItem(APP_PREFIX + 'moments_data');
        if (!compatPosts) {
          // 旧版存的是 moments_data
          const legacy = safeGetItem('moments_data');
          try { if (legacy) compatPosts = JSON.parse(legacy); } catch (e) {}
        }
        if (compatPosts && Array.isArray(compatPosts) && compatPosts.length > 0) {
          safeSetItem('akini_posts', JSON.stringify(compatPosts));
          safeSetItem('akini_posts_backup', JSON.stringify(compatPosts));
          console.log('[akini-compat-bridge] 从 localforage 恢复朋友圈帖子:', compatPosts.length);
        }
      }
      ...
      console.warn('[akini-compat-bridge] 恢复数据失败', e);
    }
  }
