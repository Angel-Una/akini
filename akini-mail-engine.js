(function () {
  ...
  function manageAutoSendTimer(forceReset) {
    ...
    // 开关与设置页「主动寄信」联动（akini_toggle_contactMailToggle），兼容旧键 akini_mailAutoSendEnabled
    var toggle = localStorage.getItem("akini_toggle_contactMailToggle");
    var legacy = localStorage.getItem("akini_mailAutoSendEnabled");
    if (toggle === "0" || legacy === "0") return;
    // 间隔与设置页「主动写信间隔」联动（akini_num_activeMailMin/Max，单位小时），兼容旧分钟键
    var minH = parseFloat(localStorage.getItem("akini_num_activeMailMin") || "");
    var maxH = parseFloat(localStorage.getItem("akini_num_activeMailMax") || "");
    var minMs, maxMs;
    if (!isNaN(minH) && minH > 0) {
      minMs = minH * 3600e3;
      maxMs = (!isNaN(maxH) && maxH >= minH ? maxH : minH) * 3600e3;
    } else {
    ...
    var now0 = Date.now();
    var delay;
    // 以上次来信时间为锚：距上次来信已超过最小间隔（离线期间错过）→ 上线后尽快补发一封
    var lastSent = parseFloat(localStorage.getItem("akini_mail_last_sent") || "0");
    if (forceReset) {
  ...
  window.akiniMailEngine = {
    ...
    // 设置页变更开关/间隔时调用：立即按新配置重排主动来信
    resetAutoSend: function () { manageAutoSendTimer(true); },
  };
