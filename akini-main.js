...
document.addEventListener("DOMContentLoaded", function () {
  try {
      ...
      (function () {
          ...
          try {
            ...
            for (var i = 0; i < localStorage.length; i++) {
              ...
              if (kk && kk.indexOf("akini_app_icon_") !== 0 && !isSnapshotKey(kk)) {
                ...
                // 大键（聊天记录/图片，>32KB）不进快照：它们有 chat_history 双备份 + IDB backupAll 兜底，
                // 否则每次快照把全量数据再复制两份塞回 localStorage，5MB 配额很快爆掉
                if (!isEmptyValue(vv) && vv.length <= 32768) snap[kk] = vv;
              }
            }
            ...
            try { localStorage.removeItem("akini_localstorage_snapshot"); } catch (e3) {}
        ...
        localStorage.setItem = function (k, v) {
          ...
          try {
            ...
            // 配额溢出：先把全部数据备份到 IndexedDB，再把聊天历史修剪为最近40条（绝不整删，防数据丢失）
            console.warn("[Akini] localStorage配额溢出,备份后修剪聊天历史重试", k);
            try {
              try { window._idbStore && window._idbStore.backupAll && window._idbStore.backupAll(); } catch (e0) {}
              for (var di = localStorage.length - 1; di >= 0; di--) {
                var dk = localStorage.key(di);
                if (dk && dk.indexOf("akini_chat_history_") === 0) {
                  try {
                    var _qh = localStorage.getItem(dk);
                    var _qt = (typeof __akiniEmergencyTrim === "function") ? __akiniEmergencyTrim(_qh, 40) : _qh;
                    if (_qt !== _qh) origSetItem.call(localStorage, dk, _qt);
                    else origRemoveItem.call(localStorage, dk); // 不足40条仍超限（大图片）才删，IDB已有完整备份
                  } catch (e1) { try { origRemoveItem.call(localStorage, dk); } catch (e2) {} }
                }
              }
              r = origSetItem.apply(this, arguments);
            } catch (e2) {
              console.error("[Akini] 清理后仍写失败", k, e2);
              throw e2;
            }
          }
    ...
    function __akiniPostLog(app, msg) {
      try {
        var arr = JSON.parse(localStorage.getItem("akini_post_log") || "[]");
        arr.unshift({ t: Date.now(), app: app, msg: msg });
        if (arr.length > 30) arr.length = 30;
        localStorage.setItem("akini_post_log", JSON.stringify(arr));
      } catch (e) {}
    }
    window.__akiniPostLog = __akiniPostLog;
    (function () {
      var jobs = {};
      window._akiniTimer = {
            ...
            if (opts && opts.keepNext) {
              ...
              if (existing) {
                if (existing > now) {
                  ...
                  // 残留"下次时间"异常偏远（超过新间隔3倍且超过6小时）视为损坏，按新间隔重排
                  if (delayMs > Math.max(freshDelay * 3, 6 * 3600 * 1000)) {
                    console.warn("[AkiniTimer] " + name + " 残留计划异常偏远，已按新间隔重排");
                    try { window.__akiniPostLog && __akiniPostLog(name, "残留计划时间损坏，已重排"); } catch (e0) {}
                    delayMs = freshDelay;
                  }
                } else {
...
((window.onerror = function (t, e, n, i, a) {
        ...
        try {
          ...
          if ("music" === t) {
            try {
              ...
              if (window._openMusicMenu) {
                window._openMusicMenu();
              }
            } catch (mErr) {
    ...
    window.__akiniFormatTimePart = function (date) {
      var e = new Date(date || Date.now());
      var hh = String(e.getHours()).padStart(2, "0");
    ...
    function __akiniPlaceReadReceipt(line, rr) {
      try {
        var b = line.querySelector(":scope > .bubble") ||
                line.querySelector(":scope > .bubble-col .bubble") ||
                line.querySelector(":scope .bubble");
        if (!b) return;
        var apply = function () {
          rr.style.top = (b.offsetTop + b.offsetHeight + 2) + "px";
          // 右缘对齐气泡右缘，再往左挪 3px
          rr.style.right = (line.clientWidth - (b.offsetLeft + b.offsetWidth) + 3) + "px";
        };
        apply();
        var imgs = b.querySelectorAll("img");
        for (var i = 0; i < imgs.length; i++) {
          if (!imgs[i].complete) imgs[i].addEventListener("load", apply, { once: true });
        }
      } catch (e) {}
    }
    ...
    function __akiniProcessMsgMeta(row) {
      ...
      if (existing) {
        // 已读回执默认恒在但隐藏，只有实际显示过（visibility:visible）才保留，不能只看文本
        var oldRr = existing.querySelector(".msg-rr");
        if (oldRr && oldRr.style.visibility === "visible") hadRead = true;
        existing.remove();
      }
      ...
      if (!showTs && !showRr) { row.setAttribute("data-meta-v", "10"); return; }
      ...
      // 时间戳 → 头像列：与气泡高度完全解耦
      if (showTs) {
        var line = row.querySelector(":scope > .msg-content-line") || row;
        var avatar = line.querySelector(":scope > .msg-avatar");
        if (avatar) {
          var col = document.createElement("div");
          col.className = "avatar-col";
          avatar.parentNode.insertBefore(col, avatar);
          col.appendChild(avatar);
          var tsEl = document.createElement("span");
          tsEl.className = "msg-ts";
          tsEl.textContent = __akiniFormatMsgTime(parseInt(ts, 10));
          col.appendChild(tsEl);
        }
      }
      // 气泡包进 bubble-wrap（宽=气泡宽），已读回执放 wrap 内：天然贴气泡正下方、右缘对齐
      var lineEl = row.querySelector(":scope > .msg-content-line");
      if (lineEl) {
    ...
    function __akiniUpgradeSurveyIcons(root) {
      ...
      root.querySelectorAll(".survey-bubble-icon").forEach(function (el) {
        if (el.innerHTML !== window.__akiniSurveyIconSVG) el.innerHTML = window.__akiniSurveyIconSVG;
      });
    }
    // 卡片状态胶囊颜色同步（按文本推断状态，覆盖历史卡片与新状态变更）
    function __akiniSyncCardStatus(root) {
      if (!root || !root.querySelectorAll) return;
      root.querySelectorAll(".tr-status, .survey-bubble-status, .shop-card-bubble-status").forEach(function (el) {
        var t = (el.textContent || "").trim();
        var st = /已收款|已接收|已付款|已回答/.test(t) ? "done" : (/退回|退还|拒收/.test(t) ? "refund" : "pending");
    ...
    function __akiniPaintTypingFloat() {
      ...
      if (!el) return;
      var chatBody = document.getElementById("chatBody");
      var t = window.akiniContacts ? window.akiniContacts.getActiveChatId() : "";
      var m = t && window.__akiniTypingMap ? window.__akiniTypingMap[t] : null;
      if (!m) {
      ...
      if (chatBody) {
        ...
        var __nb = chatBody.scrollHeight - chatBody.scrollTop - chatBody.clientHeight < 150;
        if (__nb) chatBody.scrollTop = chatBody.scrollHeight;
      }
    }
    ...
    function hideTypingBubble(t) {
      if (t) {
        ...
        for (var k in window.__akiniTypingMap) {
          ...
          m && m.timer && clearTimeout(m.timer);
        }
        window.__akiniTypingMap = {};
      }
      // 清理历史遗留的消息流内 typing 行（旧版本结构）
      var chatBody = document.getElementById("chatBody");
      if (chatBody) {
    ...
    // pending 回复状态持久化：iOS/国产 ROM 杀后台重载后内存定时器丢失，靠它补发回复
    function __akiniPersistPendingReply() {
      try {
        var m = window.__akiniPendingReplyMap || {};
        var out = {};
        Object.keys(m).forEach(function (k) {
          if (m[k] && m[k].count > 0) out[k] = { count: m[k].count, memberId: m[k].memberId || null, ts: m[k].ts || Date.now() };
        });
        var js = JSON.stringify(out);
        if (Object.keys(out).length) {
          try { localStorage.setItem("akini_pending_reply_v1", js); } catch (e) {}
          try { window._idbStore && window._idbStore.set("akini_pending_reply_v1", js); } catch (e2) {}
        } else {
          try { localStorage.removeItem("akini_pending_reply_v1"); } catch (e3) {}
          try { window._idbStore && window._idbStore.remove("akini_pending_reply_v1"); } catch (e4) {}
        }
      } catch (e) {}
    }
    // 启动/恢复时补发：把持久化的 pending 装回内存，并对每个会话重新调度回复
    function __akiniRestorePendingReply() {
      var raw = null;
      try { raw = localStorage.getItem("akini_pending_reply_v1"); } catch (e) {}
      var applyMap = function (rawStr) {
        var m = null;
        try { m = JSON.parse(rawStr || "null"); } catch (e) {}
        if (!m || typeof m !== "object") return;
        window.__akiniPendingReplyMap = window.__akiniPendingReplyMap || {};
        Object.keys(m).forEach(function (cid) {
          var p = m[cid];
          if (!p || !(p.count > 0)) return;
          // 超过 30 分钟的陈旧 pending 丢弃（对话上下文早已过去）
          if (p.ts && Date.now() - p.ts > 1800000) return;
          if (!window.__akiniPendingReplyMap[cid]) {
            window.__akiniPendingReplyMap[cid] = { count: p.count, memberId: p.memberId || null, ts: p.ts };
          }
          (function (id) {
            setTimeout(function () { try { b(id); } catch (e) {} }, 2500 + Math.random() * 2000);
          })(cid);
        });
      };
      if (raw) { applyMap(raw); return; }
      try {
        window._idbStore && window._idbStore.get("akini_pending_reply_v1", function (v) { if (v) applyMap(v); });
      } catch (e) {}
    }
    ...
    function b(t) {
      ...
      // （残留定时器/catchUp 补跑等无用户交互的触发在此被丢弃）
      var _pend = window.__akiniPendingReplyMap && window.__akiniPendingReplyMap[t];
      if (!_pend || !(_pend.count > 0)) {
        console.log("[b] 无待回复消息，丢弃残留回复任务", t);
...
// 对齐 milk：移除 45 秒节流，只要用户发消息，延时到点正常回复
    ...
    function akiniTriggerReply(t) {
      if (!window.akiniContacts) return;
      // 回复总开关：关闭后任何"对方回应"（表情包/转账/收款等后续）都不再触发
      if (window.__akiniToggleOn && !window.__akiniToggleOn("contactReplyToggle", true)) return;
      t = t || window.akiniContacts.getActiveChatId();
      ...
      (window._akiniRescueEmptyData = function () {
        ...
        try {
          ...
          if (!Array.isArray(contacts) || contacts.length === 0) {
            ...
            // vivo/oppo 等国产 ROM 重进时 localStorage/sessionStorage 可能被系统清理，
            // 同步源全部落空后从 IDB 兜底恢复（IDB 一般不会被清）
            try {
              var afterSync = window.akiniContacts.getContacts();
              if ((!Array.isArray(afterSync) || afterSync.length === 0) && window._idbStore && window._idbStore.get) {
                window._idbStore.get("akini_contacts", function (v) {
                  try {
                    var pv = typeof v === "string" ? JSON.parse(v) : v;
                    if (Array.isArray(pv) && pv.length > 0) {
                      var now2 = window.akiniContacts.getContacts();
                      if (!Array.isArray(now2) || now2.length === 0) {
                        window.akiniContacts.saveContacts(pv);
                        console.log("[抢救] 联系人从 IDB 恢复,数量:", pv.length);
                      }
                    }
                  } catch (e1) {}
                });
    ...
    /* 默认头像：线条人形 SVG（全局统一，联系人和「我」未设置头像时使用） */
    var AKINI_LINE_AVATAR_URI =
    ...
    clearWpBtn &&
      clearWpBtn.addEventListener("click", function () {
        var cid = window.akiniContacts ? window.akiniContacts.getActiveChatId() : null;
        ...
        if (cb2) {
          ...
          cb2.style.backgroundSize = "";
          cb2.style.backgroundPosition = "";
          cb2.style.backgroundRepeat = "";
          cb2.setAttribute("data-current-bg", "");
        }
        try { if (typeof Y !== "undefined" && Y) { Y.style.display = "none"; Y.style.pointerEvents = "none"; } } catch (e) {}
        try { window.akiniShowToast && window.akiniShowToast("已恢复默认壁纸"); } catch (e) {}
      });
    ...
    (me(),
      (function () {
        ...
        function wbNormalizeLegacyDump(x) {
          ...
          if (x.customReplies || x.customPokes || x.customEmojis || x.customReplyGroups) return x;
          var keys = Object.keys(x);
          var hasLegacy = keys.some(function (k) { return k.indexOf("akini_wb_") === 0; }) || typeof x.akini_wordbank === "string" || Array.isArray(x.akini_wordbank);
          if (!hasLegacy) return x;
          var parseArr = function (v) {
          ...
          var cards = parseArr(x.akini_wordbank);
          var out = { customReplies: [], customEmojis: [], customPokes: [], customReplyGroups: [] };
          var txt = function (t) { return String((t && typeof t === "object" ? (t.text || t.content) : t) || "").trim(); };
          cards.forEach(function (t) {
            var s2 = txt(t);
            if (!s2) return;
            var tab = (t && typeof t === "object" && t.tab) || "main";
            if (tab === "emoji") out.customEmojis.push(s2);
            else if (tab === "pat") out.customPokes.push(s2);
            else if (t && typeof t === "object" && t.gid) { /* 组内字卡走分组 items，避免重复 */ }
            else out.customReplies.push(s2);
          });
          var gs = parseArr(x.akini_wb_groups_main).concat(parseArr(x.akini_wb_groups));
          var seenG = {};
          gs.forEach(function (g) {
            if (!g || !g.name || seenG[g.name]) return;
            seenG[g.name] = 1;
            var items = cards
              .filter(function (t) { return t && typeof t === "object" && String(t.gid || "") === String(g.id); })
              .map(txt).filter(Boolean);
            out.customReplyGroups.push({ id: g.id, name: g.name, color: g.color, items: items });
          });
        ...
        function renderExclContacts() {
          ...
          ((exclCid = null),
            ...
            // 主页：隐藏「关闭」按钮（用户要求删除），退出走左上角「‹」返回字卡库
            (function(){var c=document.getElementById("wbExclClose");c&&(c.style.display="none");var bh=document.getElementById("wbExclBackHome");bh&&(bh.style.display="");})(),
            (WETip.textContent =
        ...
        function renderExclSeg(cid) {
          ...
          /* 阻止触摸事件冒泡到外层 fast-tap 委托容器（WEL），否则 touchend 被 WEL 拦截
             preventDefault 后 seg 按钮的原生 click 永远不会派发，表现为「点了没反应」 */
          ["touchstart", "touchend", "touchmove"].forEach(function (evn) {
            seg.addEventListener(evn, function (ev) { ev.stopPropagation(); }, { passive: true });
          });
        ...
        function __wbCollectMilkData(sel, onlyGroupIds) {
          ...
          var inGroup = {};
          (onlyGroupIds || []).forEach(function (g) { inGroup[String(g)] = 1; });
          var hasGroupFilter = onlyGroupIds && onlyGroupIds.length > 0;
          if (sel.main) {
          ...
          if (sel.emoji && !hasGroupFilter) {
            data.customEmojis = all
              .filter(function (t) { return (t && t.tab) === "emoji" && txt(t); })
              .map(txt);
            if (data.customEmojis.length) data.modules.push("emojis");
          }
          if (sel.pat && !hasGroupFilter) {
            data.customPokes = all
              .filter(function (t) { return (t && t.tab) === "pat" && txt(t); })
              .map(txt);
            if (data.customPokes.length) data.modules.push("pokes");
          }
          if (sel.groups) {
            var gs = (window.__wbRead("akini_wb_groups_main", []) || []).filter(function (g) {
              return g && g.name && (!hasGroupFilter || inGroup[String(g.id)]);
            });
            if (gs.length) {
              data.customReplyGroups = gs.map(function (g) {
                return {
                  id: g.id,
                  name: g.name,
                  color: g.color || "#74C0FC",
                  items: all
                    .filter(function (t) { return ((t && t.tab) || "main") === "main" && String(t.gid || "") === String(g.id) && txt(t); })
                    .map(txt),
                };
              });
        ...
        /* 按分组导出面板（akini 风格底部弹层） */
        function __wbShowGroupExportSheet() {
          var gs = window.__wbRead("akini_wb_groups_main", []) || [];
          var all = l() || [];
          if (!gs.length) {
            window.__akiniCenterModal && window.__akiniCenterModal("按分组导出", "还没有分组\n请先在字卡库「分组」中创建分组");
            return;
          }
          var ov = document.createElement("div");
          ov.style.cssText = "position:fixed;inset:0;z-index:1000002;background:rgba(0,0,0,.55);display:flex;align-items:flex-end;justify-content:center";
          ...
          ov.innerHTML = '<div style="background:#fff;border-radius:20px 20px 0 0;width:100%;max-width:480px;padding:18px 20px calc(18px + env(safe-area-inset-bottom,0px));max-height:80vh;overflow-y:auto">' +
            '<div style="font-size:16px;font-weight:700;color:#222;margin-bottom:2px">按分组导出</div>' +
          ...
          ov.addEventListener("click", function (ev) {
            ...
            if (btn.dataset.act === "cancel") { ov.remove(); return; }
            var gids = [];
            ov.querySelectorAll("input[type=checkbox][data-gid]").forEach(function (cb) { if (cb.checked) gids.push(cb.dataset.gid); });
            ov.remove();
        ...
        /* 导出面板（akini 风格底部弹层）：模块勾选 + 按分组导出入口 */
        window.__wbExportCards = function () {
          try {
            ...
            ov.addEventListener("click", function (ev) {
              ...
              if (!btn) return;
              if (btn.dataset.act === "cancel") { ov.remove(); return; }
              if (btn.dataset.act === "bygroup") { ov.remove(); __wbShowGroupExportSheet(); return; }
              var sel = {};
              ov.querySelectorAll("input[type=checkbox][data-key]").forEach(function (cb) { sel[cb.dataset.key] = cb.checked; });
              ov.remove();
          ...
          if (U && window.akiniContacts.getActiveChatId() === t) {
            ...
            if (
              ...
              (o.className = "msg-row system"),
                ...
                U.appendChild(o),
                (U.scrollHeight - U.scrollTop - U.clientHeight < 150) && (U.scrollTop = U.scrollHeight);
            }
          }
          "function" == typeof ot && ot();
        }
      ...
      ((function () {
        ...
        if (t) {
          ...
          ((window.openIcitySaveModal = function (e) {
            // 保存图片上的名字/别名用当前资料刷新：旧日记的 name 可能是默认名（如"对方"）
            try {
              var _aid = e && (e.authorId || e.contactId);
              ...
              } else if (!_aid || _aid === "me") {
                var _myNick = localStorage.getItem("akini_icity_my_nick");
                if (_myNick && _myNick !== e.name) e = Object.assign({}, e, { name: _myNick });
              }
            } catch (re) {}
            ((l = e),
              (s = 0),
                    ...
                    s &&
                      (s.onclick = function () {
                        ...
                        if (e) {
                          ...
                          (a.push(sentItem),
                            saveMailSent(a));
                          // 应用保持打开时立即检查一次，到点即投递回信
                          if (willReply && window.akiniMailEngine && window.akiniMailEngine.checkStatus) {
                            window.akiniMailEngine.checkStatus();
                          }
                          window.__akiniToast && window.__akiniToast(willReply ? "回信已寄出，正在加急寄给 " + (originalLetter.from || "对方") + " ✉️" : "回信已寄出 ✉️");
    ...
    ((function () {
      ...
      ((window.showInAppNotif = function (t) {
        ...
        // 「主动寄信」开关切换时联动信箱引擎立即重排（开→按新间隔排计划；关→清除待发定时器）
        (function () {
          var _mt = document.getElementById("contactMailToggle");
          if (_mt)
              ...
              setTimeout(function () {
                try {
                  if (
                    ...
                    typeof window.akiniMailEngine.resetAutoSend === "function"
                  )
                    window.akiniMailEngine.resetAutoSend();
                } catch (e) {}
              }, 50);
            });
        })(),
        ...
        (null !== n && (e.value = n),
          e.addEventListener("change", function () {
            ...
            if (t.key === "akini_num_activeMailMin" || t.key === "akini_num_activeMailMax" || t.key === "akini_num_activeMsgMin" || t.key === "akini_num_activeMsgMax" || t.key === "akini_num_friendsPostMin" || t.key === "akini_num_friendsPostMax" || t.key === "akini_num_icityPostMin" || t.key === "akini_num_icityPostMax") {
              ...
              // 信箱主动来信引擎（akini-mail-engine）按新间隔立即重排
              if (window.akiniMailEngine && typeof window.akiniMailEngine.resetAutoSend === "function") {
                try { window.akiniMailEngine.resetAutoSend(); } catch (e) {}
              }
    ...
    !(function __amt(isFirst) {
      ...
      if (!window.__akiniToggleOn || !window.__akiniToggleOn("contactActiveMsgToggle", false)) {
        return;
      }
      const e = parseFloat(
          localStorage.getItem("akini_num_activeMsgMin") || "5",
    ...
    (function () {
      ...
      function Tt(n, i, a) {
        ...
        if ("netease" !== B.platform)
          return (
            alert(
              "当前仅支持网易云音乐歌单解析，请复制网易云歌单链接或歌单ID后粘贴到此处导入",
            ),
        ...
        return _fetchTrackAll().catch(function (err) {
          ...
          return (
            a ||
              ...
              console.log("[歌单] 保存,歌曲数:", c.length),
              (l = 0),
          ...
          c =
            ...
            (function () {
              try {
                ...
                var parsed = t ? JSON.parse(t) : a;
                console.log("[歌单] 初始化读取,localStorage有:", !!t, "歌曲数:", (parsed||[]).length);
                return parsed;
              } catch (t) {
                console.warn("[歌单] 初始化读取失败", t);
                return a;
              }
            })()),
          l =
        ...
        (e.menuOverlay &&
          ...
          e.menuBtn &&
            (e.menuBtn.addEventListener("click", function (t) {
              ...
              t.stopPropagation();
              try {
                window._openMusicMenu && window._openMusicMenu();
              } catch (e) {}
            }),
      ...
      function It() {
        ...
        window._akiniMusicReloadPlaylist = function () {
          function applySaved(saved) {
            console.log("[歌单] reload applySaved,歌曲数:", (saved||[]).length);
            try {
        ...
        window._openMusicMenu = function () {
          try {
            if (typeof Nt === "function") {
              Nt();
            }
          } catch (e) {
            console.error("[music] menu open error:", e);
          }
          try {
...
(function () {
  ...
  function _jumpToMoment(app, momentId) {
    ...
    if (app === "icity") {
      /* icity：先确保进入 icity 页面与日记 tab，再按 data-diary-id 定位 */
      try { window.navTo && window.navTo("icity"); } catch (e) {}
      setTimeout(function () {
      ...
      /* 朋友圈：先进入朋友圈页面；渲染按 ts 倒序，索引需先排序再匹配 */
      try { window.navTo && window.navTo("friends"); } catch (e) {}
      setTimeout(function () {
        try {
  ...
  function _openNotifCenter(app) {
    var modal = document.getElementById(app === "icity" ? "icityNotifModal" : "friendsNotifModal");
    if (!modal) return;
    try { modal.style.setProperty("z-index", "2147483640", "important"); } catch (e) { modal.style.zIndex = "2147483640"; }
...
(function () {
  ...
  function _applyStickerChrome(on) {
    ...
    var gf = document.getElementById('wbGroupFilter');
  ...
  function _bindTabs() {
    ...
    tabs.__stkBound = 1;
    /* iOS 上原有 touchend 处理器会抑制 click，三条通道并行兜底确保 __wbTab 更新 */
    tabs.addEventListener('click', function () { setTimeout(function () { _syncTab(); }, 0); });
    tabs.addEventListener('touchend', function (e) {
      var t = e.target && e.target.closest ? e.target.closest('.tab') : null;
      if (t) {
        var name = t.getAttribute('data-tab');
        setTimeout(function () { _syncTab(name); }, 0);
        setTimeout(function () { _syncTab(); }, 80);
      }
    }, true);
  ...
  /* ---- 表情包专属添加弹窗逻辑 ---- */
  var _pendingStickerFiles = [];

  function _renderStickerPreviews() {
    var grid = document.getElementById('wbStickerPreviewGrid');
    var countEl = document.getElementById('wbStickerPickedCount');
    var clearBtn = document.getElementById('wbStickerClearAll');
    if (!grid) return;
    if (countEl) countEl.textContent = String(_pendingStickerFiles.length);
    if (clearBtn) clearBtn.style.display = _pendingStickerFiles.length > 0 ? 'inline-block' : 'none';
    if (!_pendingStickerFiles.length) {
      grid.innerHTML = '<div id="wbStickerEmptyTip" style="grid-column:1/-1;text-align:center;color:#bbb;font-size:13px;padding:36px 0;">未选择任何图片</div>';
      return;
    }
    var html = '';
    ...
    grid.innerHTML = html;
    grid.querySelectorAll('.wb-stk-del-item').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        e.preventDefault(); e.stopPropagation();
        var i = parseInt(this.getAttribute('data-idx'), 10);
        if (!isNaN(i) && i >= 0 && i < _pendingStickerFiles.length) {
          _pendingStickerFiles.splice(i, 1);
  ...
  window.__initAddStickerModal = _openAddStickerModal;
  window.__openAddStickerModal = _openAddStickerModal;

  function _bindAddStickerModalEvents() {
    var modal = document.getElementById('addStickerModal');
    if (!modal || modal.__boundStkEvents) return;
    modal.__boundStkEvents = 1;

    var input = document.getElementById('wbStickerMultiInput');
    if (input) {
      input.addEventListener('change', function () {
        var files = Array.from(this.files || []);
        this.value = '';
        ...
        files.forEach(function (file) {
          ...
          reader.onload = function (ev) {
            if (ev.target && ev.target.result) {
              _pendingStickerFiles.push(ev.target.result);
            }
            pending--;
            if (pending <= 0) {
              _renderStickerPreviews();
    ...
    var cancelBtn = document.getElementById('cancelAddSticker');
    if (cancelBtn) {
    ...
    var confirmBtn = document.getElementById('confirmAddSticker');
    if (confirmBtn) {
  ...
  /* ---- 右上角新增：表情包 tab 时弹出添加表情包弹窗 ---- */
  function _bindAdd() {
    _bindAddStickerModalEvents();
    /* iOS 上 inline onclick 可能因 touchend 抑制而不触发，这里在捕获阶段直接接管 */
    var addBtn = document.querySelector('#wordbankOverlay .wb-head-add');
    if (addBtn && !addBtn.__stkBound) {
      addBtn.__stkBound = 1;
      var openPicker = function (e) {
        ...
        e.stopPropagation();
        var now = Date.now();
        if (addBtn.__stkLock && now - addBtn.__stkLock < 700) return;
        addBtn.__stkLock = now;
        _openAddStickerModal();
      };
      addBtn.addEventListener('pointerdown', openPicker, true);
      addBtn.addEventListener('touchend', openPicker, true);
      addBtn.addEventListener('click', openPicker, true);
    }
  }
